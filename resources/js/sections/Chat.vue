...
<template>
    ...
    <div
        v-if="open"
        @click="toggleChat"
        class="fixed z-30 top-13.5 left-0 bg-black/5 w-full h-screen backdrop-blur-2xl transition-opacity duration-500"
        :class="showChat ? 'opacity-100' : 'opacity-0'"></div>
    <section
        id="chat"
        v-show="open"
        class="fixed white-glass pb-2 top-1/2 left-1/2 -translate-1/2 w-full z-30 sm:max-w-lg transition-[opacity,transform] duration-500"
        :class="showChat ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-0 rotate-180'">
        <h2 class="text-3xl py-4 text-left">{{t('chat.title')}}</h2>
