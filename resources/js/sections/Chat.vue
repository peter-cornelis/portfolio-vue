...
const inputFilled = computed(() => form.question.trim().length > 0);
const answer = ref(null);

const noticeTimeout = setTimeout(() => {
    if (!open.value) noticed.value = false;
}, 10000);

const shakeInterval = setInterval(() => {
    if (!noticed.value) shaker(false);
}, 3000);
        ...
        <section id="chat" v-if="open"
            ...
            <form @submit.prevent="submit" class="grid gap-4">
                <div class="contrastic rounded-md p-1">
                    <div
                        class="max-h-[calc(100vh-19rem)] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-xs [&::-webkit-scrollbar-thumb]:bg-white/40 overflow-y-auto">
                ...
                <label for="question">
                    {{ t('chat.ask') }}
                    <span>{{ form.errors.question ? '| ' + t(`${form.errors.question}`) : '*' }}</span>
                    <input type="text" v-model="form.question" id="question" class="contrastic w-full"
                        :class="{ 'animate-shake': shake }" :placeholder="t('chat.placeholder')">
                </label>
                <div class="relative">
                    <div @click="shaker(inputFilled)" class="absolute w-full h-full cursor-not-allowed"
                        :class="{ 'z-20': !inputFilled }"></div>
                    <button type="submit" class="relative w-full"
                        :class="!inputFilled && !form.processing ? 'btn-disabled' : 'btn-secondary'"
                        :disabled="!inputFilled || form.processing">
