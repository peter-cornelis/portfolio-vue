...
<template>
    <div class="relative not-print:hidden w-5xl">
        <div class="grid grid-cols-[1fr_auto] pt-4 px-6">
            <h1 class="w-fit text-4xl font-bold py-2 bg-teal-500 text-white px-4 rounded-md rotate-1 shadow">{{
                contact.name }}</h1>
            <p
                class="text-2xl font-bold mb-4 mt-2 bg-violet-400 text-white py-1 px-4 rounded-md w-fit -rotate-0.5 shadow">
                Full-Stack Developer</p>
            <address class="grid grid-cols-2 text-lg mb-12 mr-8">
                <ul class="grid gap-2">
                    <li class="flex items-start gap-2">
                        <house class="icon mt-1" />
                        <span class="inline-block">
                            {{ t('contact.street') }}
                            <span class="block">{{ t('contact.city') }},</span>
                            {{ t('contact.country') }}
                        </span>
                    </li>
                ...
                <ul class="grid gap-2 ml-auto mb-auto text-right">
                    <li class="flex items-center justify-end gap-2">
                        <a href="https://dev.petercornelis.be">www.petercornelis.be</a>
                        <linked class="icon" />
                    </li>
                    <li class="flex items-center justify-end gap-2">
                        <a :href="contact.linkedin">peter-cornelis-dev</a>
                        <linkedIn class="w-7 h-7 text-text/80" />
                    </li>
                    <li class="flex items-center justify-end gap-2">
                        <a :href="contact.github">peter-cornelis</a>
                        <github class="w-7 h-7 text-text/80" />
                    </li>
                    <li class="flex items-center justify-end gap-2 mt-5">
                        {{ t('contact.drive') }}
                        <car class="icon" />
                    </li>
                </ul>
            ...
            <span
                class="bg-text/80 rounded-[50%_50%_50%_10%] p-1 w-fit h-fit row-start-1 col-start-2 row-span-3 my-auto">
                <img src="../../images/profiel-15.avif" alt=""
                    class="w-64 h-64 rounded-[50%_50%_9%_50%] shadow shadow-black/20 scale-x-[-1]">
            </span>
        </div>
        <p class="px-6 py-6 mx-1 mb-4 text-lg text-primary bg-text/80 font-semibold rounded-md shadow">{{
