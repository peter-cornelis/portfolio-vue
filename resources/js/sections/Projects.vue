...
const projects = computed(() => [
    {
        ...
        githubUrl: "https://github.com/peter-cornelis/monitoring-dashboard",
        demoUrl: null
    },
    {
        name: "Dropzoned",
...
<template>
    <section id="projects" class="relative">
        <span aria-hidden="true"
            class="absolute top-5 left-1/2 -translate-x-1/2 bg-linear-to-bl from-rose-300 dark:from-rose-400 to-purple-300 w-140 h-140 rounded-full -z-10 shadow-[0px_0px_70px_15px] shadow-rose-300/25 border border-black/10 dark:border-black/40 transition-all duration-1000">
        </span>
        <span aria-hidden="true"
            class="absolute top-100 left-1/3 -translate-x-1/2 bg-linear-to-br from-rose-300 dark:from-rose-400 to-purple-400 w-60 h-60 rounded-full -z-10 shadow-[0px_0px_30px_5px] shadow-rose-300/20 border border-black/10 dark:border-black/40 transition-all duration-1000">
        </span>
        ...
        <ul class="shadow-lg shadow-black/5 glass max-w-4xl mx-auto overflow-y-clip">
            <li v-for="(project, index) in projects" :key="index" @click="setActiveProject(index)"
                ...
                :class="{ 'cursor-pointer hover:bg-indigo-700/5 dark:hover:bg-indigo-100/2 bg-indigo-500/2 dark:bg-indigo-950/5': opened !== index }">
                    ...
                    <div v-if="opened === index"><!-- extra container resolve transition glitch by padding-->
                        <div class="grid md:grid-cols-2 gap-4 p-2">
                            <div class="grid grid-cols-2 gap-4 col-span-1">
                                <img class="soft-img col-span-2" :src="project.imageUrl" :alt="project.alt">
                                ...
                                <a :href="project.githubUrl" target="_blank" rel="noopener noreferrer"
                                    class="btn-secondary mt-auto" :class="project.demoUrl ? '' : 'col-span-2'">
                            ...
                            <div class="grid justify-between milky text-sm rounded-lg p-4 mb-auto sm:col-span-1 h-full">
